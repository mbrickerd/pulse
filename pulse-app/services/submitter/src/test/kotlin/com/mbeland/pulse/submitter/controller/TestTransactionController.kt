...
import com.mbeland.pulse.submitter.config.SecurityConfig
...
class TestTransactionController {
    ...
    fun setUp() {
        ...
        justRun { rateLimitInterceptor.postHandle(any(), any(), any(), any()) }
        justRun { rateLimitInterceptor.afterCompletion(any(), any(), any(), any()) }
    }
    ...
    fun `should return 429 when rate limit is exceeded`() {
        every { rateLimitInterceptor.preHandle(any(), any(), any()) } answers {
            ...
            val body = """{"status":429,"title":"Too Many Requests","detail":"Rate limit exceeded. Please retry after a moment."}"""
    ...
    @Test
    fun `should return 503 when Kafka publish throws ExecutionException`() {
        every { transactionService.submitTransaction(any(), any()) } throws ExecutionException(RuntimeException("Kafka unavailable"))
    ...
    @Test
    fun `should return 503 when Kafka publish throws InterruptedException`() {
        every { transactionService.submitTransaction(any(), any()) } throws InterruptedException()
