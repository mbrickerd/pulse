...
import com.mbeland.pulse.projector.config.SecurityConfig
...
class TestTransactionController {
    ...
    @Test
    fun `should return 500 when an unexpected error occurs`() {
        every { findTransactionProjectionPort.findByTransactionId("tx-error") } throws RuntimeException("Unexpected")

        mockMvc.get("/transactions/tx-error") {
            with(jwt())
        }.andExpect {
