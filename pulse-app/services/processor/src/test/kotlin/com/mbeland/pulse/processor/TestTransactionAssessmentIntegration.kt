...
class TestTransactionAssessmentIntegration {
    ...
    companion object {
        ...
        @JvmStatic
        fun overrideBootstrapServers(registry: DynamicPropertyRegistry) {
    ...
    private fun createTestConsumer(topic: String): Consumer<String, String> {
        ...
        // Materialize the lazy seek before returning so the end position is captured now,
        // not at the time of the first poll() (which could be after the expected message arrives)
    ...
    fun `should publish assessed event for low-risk transaction`() {
        val consumer = createTestConsumer(Topics.TRANSACTIONS_ASSESSED)
